...
class TempPoint:
    """Dočasný bod pre dynamický preview (len POLYCURVE)"""
...
class ToolManager:
    ...
    def mousePressEvent(self, event):
        ...
        # Pravý klik: odstráni posledný bod z poly nástrojov
        ...
        if self.current_tool == "BOX":
            ...
            self.view.preview_rect = self.view.scene.addRect(
    ...
    def create_standard_path(self, points):
        ...
        if self.current_tool in ["LINE","POLYLINE"]:
            for pt in points[1:]:
                path.lineTo(pt.pos())
        else:
            for i in range(1,len(points)):
                mid = (points[i-1].pos()+points[i].pos())/2
                path.quadTo(points[i-1].pos(), mid)
        return path
